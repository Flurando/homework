...
$$\int_0^1 \int_{-\sqrt{z}}^{\sqrt{z}} \int_{-\sqrt{z-y^2}}^{\sqrt{z-y^2}} dxdydz$$
#+begin_src scheme
  ;; wished api
  ;; (integral 0 1 z
  ;;   (integral (- (sqrt z)) (sqrt z) y
  ;;     (integral (- (sqrt (- z (* y y)))) (sqrt (- z (* y y))) x 1)))
  ;; so integral should receive a lower bond, a higher bond, the dummy variable, and an integrable thing
  ;; and it should return another integrable thing
  ;; for simplicity, we will use quoted stuff to represent the integrable thing
  ;; then how to compute the integral?
  ;; by dividing, and calculating, and accumulating
  (define-syntax integral
    (syntax-rules ()
      [(_ min max sym equation)
       (_ min max sym equation 0.0001)]
      [(_ min max sym equation step)
       (let loop ((sym min))
